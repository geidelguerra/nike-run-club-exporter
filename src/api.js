...
export class NikeRunClub {
  ...
  async login() {
    ...
    await cursor.click('body')

    await cursor.click('[name="emailAddress"]', { delay: 500 })

    await page.type('[name="emailAddress"]', credentials.email, { delay: 200 })

    await page.waitForTimeout(500)

    await cursor.click('[name="password"]', { delay: 500 })

    await page.type('[name="password"]', credentials.password, { delay: 200 })
  ...
  async activities(options = {}) {
    ...
    if (afterId) {
      return await got.get(`https://api.nike.com/sport/v3/me/activities/after_id/${afterId}`, {
        headers: {
    ...
    return await got.get('https://api.nike.com/sport/v3/me/activities/after_time/0', {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    }).json()
